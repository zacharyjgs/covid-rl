...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        ob_no, ob_no_np = ptu.from_numpy(ob_no), ob_no
        ac_na, ac_na_np = ptu.from_numpy(ac_na), ac_na

        for i in range(self.num_grad_steps_per_target_update * self.num_target_updates):
            if i % self.num_grad_steps_per_target_update == 0:
                next_values = self.forward(next_ob_no) * (1 - terminal_n)
                target_values = reward_n + self.gamma * next_values
                cur_loss = self.loss(self.forward(ob_no), target_values)
                
                self.optimizer.zero_grad()
                cur_loss.backward()

            # loss.backward(retain_graph=True)  # note: this might slow down backprop
            
            self.optimizer.step()
            # loss = self.critic_network.update(ob_no_np, ac_na_np, ptu.to_numpy(target_values))
