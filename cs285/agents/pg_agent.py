...
import cs285.infrastructure.utils as ptu
...
class PGAgent(BaseAgent):
    ...
    def calculate_q_vals(self, rewards_list):
        ...
        if not self.reward_to_go:
            # For each point (s_t, a_t), associate its value as being the 
            # discounted sum of rewards over the full trajectory
            ...
            # note: this should be a 2-d array, indexed by (s_t, a_t)

        # Case 2: reward-to-go PG
        # Estimate Q^{pi}(s_t, a_t) by the discounted sum of rewards starting from t
        else:
            # For each point (s_t, a_t), associate its value as being the 
            # discounted sum of rewards over the full (remaining??) trajectory
    ...
    def estimate_advantage(self, obs, q_values):
        ...
        if self.nn_baseline:
            print("Q Val shape: {}".format(q_values.shape))
            ...
            ## done? TODO: compute advantage estimates using q_values and baselines
        ...
        if self.standardize_advantages:
            ## done? TODO: standardize the advantages to have a mean of zero
            ## and a standard deviation of one
            ## HINT: there is a `normalize` function in `infrastructure.utils`
            mean, std = np.mean(advantages), np.std(advantages)
            advantages = ptu.normalize(advantages, mean, std)
    ...
    def _discounted_return(self, rewards):
        ...
        disc_return = sum(
            [
                np.power(self.gamma, t) * rewards[t] for t in range(len(rewards))
            ]
        )
        list_of_discounted_returns = [disc_return for _ in range(len(rewards))]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        num_rews = len(rewards)
        list_of_discounted_cumsums = []
        for t in range(num_rews):
            sum_at_t = sum(
                [
                    np.power(self.gamma, i - t) * rewards[i] for i in range(t, num_rews)
                ]
            )
            list_of_discounted_cumsums.append(sum_at_t)
